namespace nl.flukeyfiddler.Utils
{
    ...
    // Plugins should contain classes implementing this type and decorated with [Export(typeof(...))]
    public class PluginWatcher<T> : IPluginWatcher<T>
    {
        ...
        private void RefreshPlugins()
        {
            ...
            {
                Logger.LogLine("Could not find plugins: " + ioex);
            }
            catch (ChangeRejectedException rejex)
            {
                Logger.LogLine("Could not update plugins: " + rejex.Message);
            }
        }
